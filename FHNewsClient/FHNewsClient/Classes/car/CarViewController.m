...
-(void)loadNewsData
{
    ...
    [manager GET:kStringCarURL parameters:nil
    ...
    success:^(AFHTTPRequestOperation *operation, NSArray *responseObject)
    {
        ...
        [responseObject enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
            ...
            [item enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
                ...
                IFCarEntity *carEntity = [[IFCarEntity alloc]initWithDic:obj];
                
                [self.entityArray addObject:carEntity];
            }];
...
-(CustomtableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CustomtableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    ...
    //3.设置单元格对象的内容
    IFCarEntity *carEntity = [self.entityArray objectAtIndex:indexPath.row];
    [carEntity caleAllFrame];
    [cell setCarEntity:carEntity];
...
{
    IFCarEntity *carEntity = [self.entityArray objectAtIndex:indexPath.row];
    if (carEntity.style == nil)
    {
       return CELL_SINGLE_PHOTO_HEIGHT  + carEntity.photoHeight;
    }
    else
    {
       return CELL_MANY_PHOTO_HEIGHT + carEntity.titleHeight + carEntity.photoHeight +carEntity.comTimeHeight;
    }
}
//返回表头
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    CustomHeadView *headView = [[CustomHeadView alloc]initWithFrame:CGRectMake(0, SEARCHBAR_HEIGHT, ScreenWidth, 200)];
    headView.pagingEnabled = YES;
    headView.contentSize = CGSizeMake(CAR_COUNT * ScreenWidth, 200);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.view.window.rootViewController presentViewController:detailVC animated:YES completion:^{
        ...
        [[NSNotificationCenter defaultCenter]postNotificationName:@"DetailVC" object:carEntity];
